...
- Name: smart-deposit.spec.ts >> Smart Deposit >> stale prices warning shown when flag is set
- Location: tests/e2e/smart-deposit.spec.ts:103:3
...
Call log:
  - waiting for locator('select').first()
    - locator resolved to <select class="rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm bg-white dark:bg-gray-800"></select>
