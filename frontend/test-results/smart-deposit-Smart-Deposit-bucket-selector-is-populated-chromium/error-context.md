...
- Name: smart-deposit.spec.ts >> Smart Deposit >> bucket selector is populated
- Location: tests/e2e/smart-deposit.spec.ts:62:3
...
Locator: locator('select').first().locator('option').filter({ hasText: 'Retirement' })
Expected: attached
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('select').first().locator('option').filter({ hasText: 'Retirement' })
