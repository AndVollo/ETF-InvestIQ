...
// ---------- Sidebar ----------
const NAV_ITEMS = [
  { id: "dashboard", icon: "home", group: "portfolio" },
  { id: "buckets", icon: "bucket", group: "portfolio" },
  { id: "deposit", icon: "deposit", group: "portfolio" },
  { id: "universe", icon: "universe", group: "tools" },
  { id: "architect", icon: "architect", group: "tools" },
  { id: "sectors", icon: "sectors", group: "tools" },
  { id: "drawdown", icon: "drawdown", group: "tools" },
  { id: "audit", icon: "audit", group: "tools" },
  { id: "settings", icon: "settings", group: "system" },
];

function Sidebar({ active, onNavigate, bucketName, bucketHorizon, onBucketClick }) {
  const { t } = useI18n();
  const portfolio = NAV_ITEMS.filter(n => n.group === "portfolio");
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="sidebar__bucket">
        <button className="bucket-trigger" onClick={onBucketClick}>
          <div className="bucket-trigger__icon">
            <Icon name="bucket" size={14} />
          </div>
          <div className="bucket-trigger__body">
            <div className="bucket-trigger__name">{bucketName}</div>
            <div className="bucket-trigger__sub">{bucketHorizon}</div>
          </div>
          <Icon name="chevronDown" size={14} className="bucket-trigger__chevron" />
        </button>
      </div>
      <nav className="sidebar__nav">
        <div className="nav-section">{t("nav.portfolio")}</div>
        {portfolio.map(item => (
          <button key={item.id}
            className="nav-item"
      ...
      <div className="sidebar__footer">
        <div className="sidebar__user">
          <div className="sidebar__user-avatar">YK</div>
          <div className="sidebar__user-body">
            <div className="sidebar__user-name">Yuval K.</div>
            <div className="sidebar__user-email">yuval@iqinvest.eth</div>
          </div>
        </div>
      </div>
    </aside>
...
// ---------- Badge for valuation ----------
function ValuationBadge({ valuation, lang }) {
  const { t } = useI18n();
  const map = {
    cheap: { cls: "badge--success", label: t("valuation.cheap") },
    fair: { cls: "badge--muted", label: t("valuation.fair") },
    expensive: { cls: "badge--warning", label: t("valuation.expensive") },
    insufficient: { cls: "badge--info", label: t("valuation.insufficient") },
  };
  const cfg = map[valuation] || map.fair;
  return (
...
window.IQ_UI = { Icon, Sidebar, Topbar, ValuationBadge, NAV_ITEMS };
