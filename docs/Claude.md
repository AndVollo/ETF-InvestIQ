...
18. **אל תשנה את base URL ב-`api/client.ts`** — חייב לתמוך גם dev (Vite proxy) וגם production (`http://127.0.0.1:8000` sidecar)
...
1. בדוק שה-binary נבנה: `ls frontend/src-tauri/binaries/`
2. בדוק את הלוגים: `cargo tauri dev` מציג הכל ב-stdout (קידומת `[backend]`)
3. נסה להריץ ידנית: `./frontend/src-tauri/binaries/smart-etf-backend-<triple>`
4. אם PyInstaller נכשל — בדוק `--hidden-import` ב-`backend/build_sidecar.py`
5. בעיות פורט (8000 תפוס): שנה `PORT` env var (לעת עתה ה-sidecar רץ על 8000)

### Tauri build נכשל

1. ודא ש-Rust מעודכן: `rustup update`
2. נקה cache: `cd frontend/src-tauri && cargo clean`
3. בדוק שהאייקונים קיימים ב-`icons/` (32x32.png, 128x128.png, icon.icns, icon.ico)
